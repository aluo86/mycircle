...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  test "that creating a friendship works without raising an exception" do
    assert_nothing_raised do
      UserFriendship.create user: users(:andrew), friend: users(:nancy)
    end
  end

  test "that creating a friendship based on user id and friend id works" do
    UserFriendship.create user_id: users(:andrew).id, friend_id: users(:nancy).id
    assert users(:andrew).pending_friends.include?(users(:nancy))
  end

  context "a new instance" do
    setup do
      @user_friendship = UserFriendship.new user: users(:andrew), friend: users(:nancy)
    end
  ...
  context "#send_request_email" do
    setup do
      @user_friendship = UserFriendship.create user: users(:andrew), friend: users(:nancy)
    end
  ...
  context "#mutual_friendship" do
    setup do
      UserFriendship.request users(:andrew), users(:yimin)
      @friendship1 = users(:andrew).user_friendships.where(friend_id: users(:yimin).id).first
      @friendship2 = users(:yimin).user_friendships.where(friend_id: users(:andrew).id).first
    end
  ...
  context "#accept!" do
    setup do
      @user_friendship = UserFriendship.request users(:andrew), users(:nancy)
    end
    ...
    should "include the friend in the list of friends" do
      ...
      assert users(:andrew).friends.include?(users(:nancy))
    end
  ...
  context ".request" do
    should "create two user friendships" do
      assert_difference 'UserFriendship.count', 2 do
        UserFriendship.request(users(:andrew), users(:nancy))
      end
    end
  ...
  context "#block!" do
    ...
    should "not allow new requests once blocked" do
      @user_friendship.block!
      uf = UserFriendship.request users(:andrew), users(:nancy)
      assert !uf.save
    end
  end
end
